...
Invoke the `ultra:launcher` skill via the Skill tool, passing `$ARGUMENTS` through verbatim. The skill's SKILL.md parses tier flags, `--ask`, `--task`, `--mode`, and the optional wrapped-skill prefix, then composes the orchestrator prompt per `${CLAUDE_SKILL_DIR}/phases.md` + `debate-protocol.md` + `anti-slop-rules.md` + `devil-advocate.md` + `tier-config.md`.
