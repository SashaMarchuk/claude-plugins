...
Writes `~/.claude/shared/identity.json` — **shared with `/g-event`**. Read-only for every subsequent skill that needs user + teammates.
...
5. **If switching workspaces** (the user had a different `workspace` before): any teammates in `identity.json` whose `external_ids.clickup` is NOT in the new workspace's member list get `active: false`. Surface this as a banner. Do not delete — other skills (like `/g-event`) still use the email + name.
...
4. **Fetch new workspace members**. For each teammate in `~/.claude/shared/identity.json`:
   ...
   - If their `external_ids.clickup` is NOT in the new workspace → set `active: false`, keep all other fields (email, name, `/g-event` still uses them).
