...
## Quick install — get all seven plugins
...
| [log-time](plugins/log-time) *(beta)* | `/log-time [range]` / `:onboard` / `:config` / `:status` — reconstruct what you actually worked on and emit a paste-ready time-log for Redmine (or any tracker your config describes). Cross-checks whatever evidence sources you configure — calendar, task tracker, Claude Code session history, meeting transcripts, spreadsheets, any custom source you can describe in plain text — via parallel sonnet sub-agents. Behavior lives in a **free-form markdown config** (`~/.claude/log-time/config.md`): your sources, target tickets, day rules, and output style, written by a four-step skippable wizard (forced on first run — it never builds a log unconfigured) and edited any time in natural language via `:config`. **Evidence-only by default** — no fabricated hours, no assumed daily target (set an optional steady target if you always log a fixed number). Pairs well with `find-call` for transcript-depth call context. **Read-only against every source** — writes only its own config and per-run artifacts. |
| [ig-dm-export-analyzer](plugins/ig-dm-export-analyzer) *(beta)* | `/ig-dm-export-analyzer:run` — turn an Instagram "Download Your Information" export into a clean, analysable chat corpus: one JSON file per conversation plus a filterable `index.json` rollup. Repairs Meta's UTF-8-as-Latin-1 mojibake, merges `message_N.json` pagination, filters strictly by date range and excluded threads, resolves media across split text/media archives, and transcribes voice notes and video audio tracks through a configurable STT provider (Soniox cloud, local whisper.cpp, or none). Every machine transcription is labeled **four independent ways** so it can never be mistaken for typed text, and a self-consistency verifier reconciles the output against the raw export. Fully config-driven — a new export is a new config file, not a code change. **Never writes into the export**; results go to the `output.dir` you choose. |
| [harness](plugins/harness) *(beta)* | `/harness:run` / `:onboard` / `:init` / `:add` / `:status` / `:stop` / `:resume` — autonomous orchestration harness: executes your ticket queue (GitHub issues or local files) end-to-end in real terminal sessions, unattended. Deterministic bash engine; model OR-chains (`fable\|mythos\|opus`); mandatory readiness **grill gate** before any ticket runs; rate-limit **pause-not-stop** with API-timed auto-resume; graceful terminal lifecycle (never force-closes, never touches sessions it didn't start). One account by default — any switcher pluggable via one config key. GSD-flavored workers out of the box. |
...
| `~/.claude/harness/config.json` + `<project>/.harness/` | User-level harness defaults (terminal, layout, accounts hook, limit policy, model chains) · project config, ticket queue, and per-run state/reports | `harness` |
